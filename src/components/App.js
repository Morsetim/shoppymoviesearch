...
class App extends React.Component {
    state = { movies: [], selectedMovieList: [], text: '' };


    onTermSubmit = async movie => {
        const response = await omdb(movie)
        if (response) { this.setState({ movies: response.data.Search, selectedMovieList: [], text:movie }) }        
    }
    ...
    render() {
        const {selectedMovieList, text} = this.state;
        return (
            <div>
            <h1>The Shoppies</h1>
              ...
              <div>
                {this.state.selectedMovieList.map(movie => {
                  return ( 
                      ...
                      <div className="mini-div">
                      <p>{movie.Title} ({movie.Year})</p><br />
                       <button id="remove" onClick={() => {this.onDeleteMovie(movie.imdbID)
